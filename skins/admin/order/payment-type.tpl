<?php if(isset($_REQUEST['add'])){ ?>
  <div class="section-interface-k2">
    <div class="line-custom-edit">
      ...
      <a class="back-url" href="/admin/order/payment-type/"><?=$messG['Назад']?></a>
    </div>
...
<?php } else { ?>
  <div class="section-interface-k1">
    <div class="line-custom">
      <p class="header-name"><?=$mess['Тип оплати']?></p>
    </div>

    <form action="" method="post" onsubmit="return okFrom();">
      <div class="line-custom-next">
        <a href="/admin/order/payment-type/?add=ok" class="add-el icon-plus"><?=$messG['Створити елемент']?></a>
      ...
      <table class="illustration-table">
        ...
        <?php if($payment['result_DB']->num_rows > 0){
          while($arResult = hsc($payment['result_DB']->fetch_assoc())){ ?>
            <tr>
              <td><input type="checkbox" name="ids[]" value="<?=$arResult['id']?>"></td>
              <td class="relative">
                <span class="icon-mob-menu" onclick="openMenuUpdate(this);"></span>
                <div class="menu-update">
                  <a href="/admin/order/payment-type/?edit=<?=$arResult['id']?>"><?=$messG['Редагувати']?></a>
                  <a href="/admin/order/payment-type/?del=<?=$arResult['id']?>"><?=$messG['Видалити']?></a>
                </div>
              </td>
