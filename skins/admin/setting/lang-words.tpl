  ...
  <form class="content-form" action="" method="post">
    ...
    <div class="tabs active-block-tabs">
      ...
      <div class="input-value">
        ...
        <select name="lang">
          <option value="ua" <?=((isset($_POST['lang']) && $_POST['lang'] == 'ua')? 'selected' : '')?>>ua</option>
          <option value="ru" <?=((isset($_POST['lang']) && $_POST['lang'] == 'ru')? 'selected' : '')?>>ru</option>
          <option value="en" <?=((isset($_POST['lang']) && $_POST['lang'] == 'en')? 'selected' : '')?>>en</option>
        </select>
      </div>
    </div>
    ...
    <?php if(isset($words) && count($words) > 0){ ?>
      <div class="technik-off-onn">
        ...
        <?php foreach($words as $section => $v){ ?>
          <div class="header-line"><?=$section?></div>
          <code class="word-lang-code"><?=wtf($v,1)?></code>
        <?php } ?>
      </div>
    <?php } ?>
