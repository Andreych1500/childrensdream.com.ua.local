                ...
                <div class="anons-characteristic">
                  ...
                  <div>
                    ...
                    <?php if ((int)$el['availability'] == 1) {
                      ...
                      <div class="add-shop <?=(!empty($basket)? $basket : '')?>"<?php if (empty($basket)) { ?> onclick="addToCard(<?=(int)$el['id']?>,'<?=$mess['BASKETOK']?>','1', this)"<?php } ?>>
          ...
          <div class="slider-nav <?=((count($photos) > 3)? 'center-photo' : 'currenc')?>">
              <?php foreach ($photos as $key => $value) { ?>
                <div class="items-photo">
                  <img src="<?=hsc($value[0])?>" alt="<?=hsc($arResult['img_seo_alt_'.$lang])?>">
                </div>
              <?php } ?>
          </div>
