...
function deleteElement($ids, $db_table, $url, $messG) {
    ...
    if (!$arSel->num_rows) {
        sessionInfo($url, 'Error deleting, ID not exist!');
    }
...
function deactivateElement($ids, $db_table) {
    q(" 
        UPDATE `".$db_table."` 
        SET `active` = 0
        WHERE `id` IN (".$ids.")
...
if (isset($_GET['route'])) {
    ...
    if (in_array($temp[0], Core::$LINK_LANG) && $temp[0] != 'ua') {
        $lang = $temp[0];
        $link_lang = '/ru/';
    } else {
        $lang = 'ua';
    ...
    if ($temp[0] == 'admin') {
        ...
        Core::$SKIN = 'admin';
        unset($temp[0]);
    } elseif ((in_array($temp[0], Core::$LINK_LANG) && $temp[0] != 'ua')) {
        unset($temp[0]);
    }
